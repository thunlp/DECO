...
def swanlab_login():
    if not os.path.exists("swanlab_key"):
        print("WARNING: swanlab_key file not found, skipping swanlab login")
        return
    with open("swanlab_key", "r") as f:
...
def _set_wandb_writer(args):
    ...
    if getattr(args, 'wandb_project', '') and args.rank == (args.world_size - 1):
        ...
        os.makedirs(wandb_kwargs['logdir'], exist_ok=True)
        if args.wandb_mode in ["cloud", "online"]:
            swanlab_login()
        swanlab.init(**wandb_kwargs)
        _GLOBAL_WANDB_WRITER = swanlab
