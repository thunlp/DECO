...
def load_checkpoint(ddp_model, optimizer, opt_param_scheduler, load_arg='load', strict=True,
    ...
    if not skip_load_to_model_and_opt:
        if len(ddp_model) == 1:
            mk, uk = ddp_model[0].load_state_dict(state_dict['model'], strict=False)
            missing_keys += mk
            unexpected_keys += uk
        else:
            for i in range(len(ddp_model)):
                mpu.set_virtual_pipeline_model_parallel_rank(i)
                mk, uk = ddp_model[i].load_state_dict(state_dict['model%d' % i], strict=False)
                missing_keys += mk
                unexpected_keys += uk
    new_missing_keys = []
    for key in missing_keys:
        if "proxy_experts" in key:
            print_rank_0(f"WARNING: Missing key {key} when loading checkpoints.")
        else:
            new_missing_keys.append(key)
    missing_keys = new_missing_keys
    if strict and (len(missing_keys) > 0 or len(unexpected_keys) > 0):
        raise RuntimeError(f"Load checkpoint error, missing keys: {missing_keys}; "
