...
from megatron.core.transformer.moe.expert_metrics import track_expert_health_metrics
    ...
    def get_make_graphed_callables_kwargs():
        ...
        def get_fp8_recipe(config):
            ...
            if config.fp8:
                ...
                from megatron.core.transformer.extensions.transformer_engine import (
                    TEDelayedScaling,
                )
...
def pretrain(
    ...
    if 'PYTORCH_CUDA_ALLOC_CONF' in os.environ:
        print("ERROR", os.environ['PYTORCH_CUDA_ALLOC_CONF'])
        raise ValueError("can not set this")
    initialize_megatron(
    ...
    if args.data_path is not None:
        config.dataset_path_list = args.data_path[1::2]
...
def setup_model_and_optimizer(model_provider_func,
    ...
    # mup optimizer
    if args.use_mup:
        lr_mult /= (args.hidden_size / args.mup_base_hidden_size)
        def scale_lr_cond(name, param):
            if name.endswith(".weight") and "router_norm" not in name and "expert_act" not in name \
                    and "layer_norm" not in name and "layernorm" not in name \
                    and "embedding" not in name and "output_layer" not in name:
                return True
            else:
                return False
    else:
...
def train_step(forward_step_func, data_iterator,
    ...
    if config.moe_router_enable_expert_bias:
        for model_chunk in model:
            LossFreeBalance.print_expert_bias(args.curr_iteration, model_chunk)

    tokens_per_expert_list = []
    router_layer_numbers = []
    for model_chunk in model:
        for module in get_attr_wrapped_model(model_chunk, 'modules')():
            if hasattr(module, 'local_tokens_per_expert') and module.local_tokens_per_expert is not None and not torch.all(module.local_tokens_per_expert == 0):
                tokens_per_expert_list.append(module.local_tokens_per_expert)
                router_layer_numbers.append(module.layer_number)
    if len(tokens_per_expert_list) > 0:
        stacked_tokens_per_expert = torch.stack(tokens_per_expert_list, dim=0)
        mean_tokens = stacked_tokens_per_expert.sum(dim=-1, keepdim=True) / stacked_tokens_per_expert.shape[-1]
        max_violation = torch.max(torch.abs(stacked_tokens_per_expert - mean_tokens), dim=-1, keepdim=True)[0] / mean_tokens
        max_violation = max_violation.squeeze()
        assert len(max_violation) == len(router_layer_numbers)
        for max_vio, layer_number in zip(max_violation, router_layer_numbers):
            save_to_aux_losses_tracker("max_violation", max_vio, layer_number, config.num_layers)
        for tokens_per_expert in tokens_per_expert_list:
            tokens_per_expert.zero_()

    activation_num, total_num, activation_rate = ActivationStatistics.get_clear_activation_num()
    stat_dict = ActivationStatistics.get_clear_layer_values()

    if RegRouterEntropy.is_enabled():
        router_entropy = RegRouterEntropy.get_clear_router_entropy()
        reg_coef = RegRouterEntropy.get_reg_coef()
        RegRouterEntropy.step_reg_coef(activation_rate, 100 * args.router_target_act_ratio)
    else:
        router_entropy, reg_coef = 0., 0.

    act_info = {
        "activation_num": activation_num, "total_num": total_num,
    ...
    if mpu.is_pipeline_last_stage(ignore_virtual=True):
        ...
        if "task_num_list" in losses_reduced[0] and "loss_list" in losses_reduced[0]:
            task_nums = torch.cat([x["task_num_list"] for x in losses_reduced]).sum(dim=0)
            task_losses = torch.cat([x["loss_list"] for x in losses_reduced]).sum(dim=0)
            loss_reduced = {"lm_loss/" + config.dataset_path_list[key].split("/")[-1]: task_losses[key] / task_nums[key] for key in torch.nonzero(task_nums)}
        else:
            loss_reduced = {}

        for key in losses_reduced[0].keys():
            if key not in ["lm loss"]:
                continue
            numerator = 0
            denominator = 0
        ...
        return loss_reduced, skipped_iter, should_checkpoint, should_exit, exit_code, grad_norm, num_zeros_in_grad, act_info
    return {}, skipped_iter, should_checkpoint, should_exit, exit_code, grad_norm, num_zeros_in_grad, act_info


def training_log(loss_dict, total_loss_dict, learning_rate, decoupled_learning_rate, iteration,
                 loss_scale, report_memory_flag, skipped_iter,
                 grad_norm, params_norm, num_zeros_in_grad, act_info):
    ...
    activation_rate = act_info.get("activation_rate", None)
    router_entropy = act_info.get("router_entropy", None)
    reg_coef = act_info.get("reg_coef", None)
    stat_dict = act_info.get("stat_dict", None)
    ...
    if writer and (iteration % args.tensorboard_log_interval == 0):
        ...
        if activation_rate is not None:
            writer.add_scalar('activation-rate', activation_rate, iteration)
            writer.add_scalar('activation-rate vs samples', activation_rate, args.consumed_train_samples)
            router_entropy_key = "l1-norm" if args.router_type == "remoe" else "router-entropy"
            writer.add_scalar(router_entropy_key, router_entropy, iteration)
            writer.add_scalar(f'{router_entropy_key} vs samples', router_entropy, args.consumed_train_samples)
            writer.add_scalar('reg-coef', reg_coef, iteration)
            writer.add_scalar('reg-coef vs samples', reg_coef, args.consumed_train_samples)
        if stat_dict is not None:
            for key, val in stat_dict.items():
                writer.add_scalar(key, val, iteration)
                writer.add_scalar(f'{key} vs samples', val, args.consumed_train_samples)
    if args.num_experts is not None:
        ...
        if getattr(args, "moe_track_expert_metrics", False):
            track_expert_health_metrics(
                iteration=iteration,
                writer=writer,
                wandb_writer=wandb_writer,
                num_layers=args.num_layers,
                ...
                ) or args.ffn_hidden_size,
                output_feature_dim=args.hidden_size,
                per_layer_logging=getattr(
                    args, "moe_expert_metrics_per_layer_logging", False
                ),
                per_expert_logging=getattr(
                    args, "moe_expert_metrics_per_expert_logging", False
                ),
            )
    if args.mtp_num_layers is not None:
    ...
    if iteration % args.log_interval == 0:
        ...
        if activation_rate is not None and wandb_writer:
            wandb_writer.log({'activation-rate': activation_rate}, iteration)
            router_entropy_key = "l1-norm" if args.router_type == "remoe" else "router-entropy"
            wandb_writer.log({router_entropy_key: router_entropy}, iteration)
            wandb_writer.log({'reg-coef': reg_coef}, iteration)
        if stat_dict is not None and wandb_writer:
            for key, val in stat_dict.items():
        ...
        if activation_rate is not None:
            router_entropy_key = "l1 norm" if args.router_type == "remoe" else "router entropy"
            log_string += ' activation rate: {:2f} |'.format(activation_rate)
            log_string += f' {router_entropy_key}: {router_entropy:6f} |'
            log_string += ' reg coef: {:6f} |'.format(reg_coef)
        total_loss_dict[advanced_iters_key] = 0
...
def checkpoint_and_decide_exit(model, optimizer, opt_param_scheduler, iteration,
    ...
    if (args.exit_interval and iteration % args.exit_interval == 0) or \
        (hasattr(args, 'exit_iteration') and iteration == args.exit_iteration):
        if args.save and not saved_checkpoint:
            save_checkpoint_and_time(iteration, model, optimizer,
                                     opt_param_scheduler,
                                     num_floating_point_operations_so_far,
    ...
    if args.early_exit_indicator_file:
        if os.path.exists(args.early_exit_indicator_file):
            with open(args.early_exit_indicator_file, 'r') as f:
                content = f.read().strip()
            # If the file content is an int, set train_iters to that value
            if content.isdigit():
                if not hasattr(args, 'exit_iteration') or args.exit_iteration != int(content):
                    args.exit_iteration = int(content)
                    print_datetime(f'set exit_iteration to {args.exit_iteration}, according to the early exit indicator file: {args.early_exit_indicator_file}')
            else:
                print_datetime(f"Error parsing early_exit_indicator_file: {content}")
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    while iteration < args.train_iters:
        ...
        loss_dict, skipped_iter, should_checkpoint, should_exit, exit_code, grad_norm, num_zeros_in_grad, act_info = \
