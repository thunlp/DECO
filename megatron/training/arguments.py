...
def add_megatron_arguments(parser: argparse.ArgumentParser):
    ...
    parser = _add_blockffn_args(parser)
...
def _add_network_size_args(parser):
    ...
    group.add_argument('--rope-type', type=str, choices=['longrope', 'llama', 'yarn'])
    group.add_argument('--longrope-long-factor', nargs="*", default=None, type=float,
                        help='long factor used in longrope')
    ...
    group.add_argument('--use-mup', action='store_true', help='Enable mup(Maximal Update Parameterization)')
    group.add_argument('--mup-emb-scale', type=float, default=1, help='mup parameterization factor for embedding')
    group.add_argument('--mup-depth-scale', type=float, default=1, help='mup parameterization factor for model depth')
    group.add_argument('--mup-base-hidden-size', type=int, default=256, help='hidden size of small model used in mu-transfer')
...
def _add_logging_args(parser):
    ...
    group.add_argument('--wandb-mode', type=str, default='cloud', choices=["disabled", "cloud", "local", "offline"],
                       help='The mode of Wandb / SwanLab.')
...
def _add_training_args(parser):
    ...
    group.add_argument('--exit-interval', type=int, default=None,
                       ...
                       'by this value.')
    group.add_argument('--early-exit-indicator-file', type=str, default=None,
                       help='Exit the program if the file shows the training steps.')
    ...
    group.add_argument('--use-span-based-attn', action='store_true', 
                       help='use span-based attn.')
...
def _add_distributed_args(parser):
    ...
    group.add_argument('--encoder-tensor-model-parallel-size', type=int, default=1,
...
def _add_moe_args(parser):
    ...
    group.add_argument('--moe-router-score-function', type=str,
                       choices=['softmax', 'sigmoid', 'sigmoid_with_prior'],
                       default='softmax',
                       help='Score function for MoE TopK routing. Can be "softmax" or "sigmoid".')
    group.add_argument('--moe-router-topk', type=int, default=2,
                       help='Number of experts to route to for each token. The default is 2.')
    group.add_argument('--moe-router-topp', type=float, default=0.5,
                       help='Accumulated scores of experts to route to for each token. The default is 0.5.')
    ...
    group.add_argument('--moe-track-expert-metrics', action='store_true',
                       help='Instrument routed experts to log per-expert MoE health metrics: '
                       'mean RMS of the SwiGLU intermediate activation and Frobenius norms of '
                       'the expert projection matrices. Per-layer max/median and min/median '
                       'dispersion ratios are reported through the standard TensorBoard / W&B '
                       'logging path. These metrics catch silent expert collapse and localized '
                       'activation blow-up (Step 3.5 Flash, arXiv:2602.10604, sections 4.1.2 / 4.1.3) '
                       'that router-dispatch statistics and the training loss both miss.')
    group.add_argument('--moe-expert-metrics-per-layer-logging', action='store_true',
                       help='When --moe-track-expert-metrics is set, additionally emit one scalar '
                       'per MoE layer (in addition to the cross-layer mean).')
    group.add_argument('--moe-expert-metrics-per-expert-logging', action='store_true',
                       help='When --moe-track-expert-metrics is set, additionally emit one scalar '
                       'per (layer, expert). Very verbose — intended for offline diagnosis.')
    ...
    group.add_argument('--moe-expert-bias-update-method', type=str, default="sgd_sign", choices=["sgd_sign"],
                       help='The update method for the expert bias used in loss-free load balancing.')
    group.add_argument('--moe-expert-bias-apply-method', type=str, default="base", choices=["base", "rms"],
                       help='The apply method for the expert bias used in loss-free load balancing.')
    group.add_argument("--print-expert-bias-step", type=int, default=-1,
                       help="The interval to print the expert load.")
    group.add_argument("--expert-activation-norm", type=str, choices=["null", "gate", "up", "gate_up", "mul", "down"],
                       default="null", help="The normalization method applied to the expert activations.")
    group.add_argument("--expert-not-gated", action="store_true",
                       help="Whether to make each MoE expert a non-gated FFN.")
    group.add_argument("--router-type", type=str, choices=["topk", "blockffn", "remoe", "topp"],
                       default="topk", help="The type of router to use in MoE layers.")
    group.add_argument("--dense-mixer-enabled", action="store_true",
                       help="If set, use the DenseMixer for training.")
    return parser


def _add_blockffn_args(parser):
    group = parser.add_argument_group(title="blockffn")
    group.add_argument("--use-blockffn", action='store_true', help="Whether to use BlockFFN.")
    group.add_argument("--router-act-func", type=str, default="relu",
                       help="The activation function of router.")
    group.add_argument("--router-norm-type", type=str, choices=["rms", "normal", "simple", "null"], default="simple",
                       help="The type of normalization after router.")
    group.add_argument("--router-norm-init-var", type=float, default=0.1,
                       help="The init value of normalization after router.")
    group.add_argument("--router-norm-fixed", action="store_true",
                       help="Whether to use a fixed normalization factor after the router.")
    group.add_argument("--router-norm-scalar", action="store_true",
                       help="Whether to use the scalar normalization factor after the router.")
    group.add_argument("--expert-act-func", type=str, default="norm_silu",
                       help="The activation function of experts.")
    group.add_argument("--expert-act-norm-type", type=str, choices=["rms", "normal", "simple", "null", "block_normal"], default="normal",
                       help="The type of normalization inside expert activation functions.")
    group.add_argument("--expert-act-norm-init-var", type=float, default=1.0,
                       help="The init value of the normalization inside expert activation functions.")
    group.add_argument("--router-target-act-ratio", type=float, default=0.2,
                       help="The target activation ratio of router.")
    group.add_argument("--router-clamp-act-ratio", type=float, default=1.0,
                       help="The maximum activation ratio allowed.")
    group.add_argument("--router-entropy-loss-coeff", type=float, default=0.,
                       help="The initial value of the router entropy loss coefficient.")
    group.add_argument("--router-entropy-loss-coeff-resume", type=float, default=0.,
                       help="The resumed value of the router entropy loss coefficient.")
    group.add_argument("--router-entropy-loss-coeff-multiplier", type=float, default=0.,
                       help="The scaling factor of the router entropy loss coefficient.")
    group.add_argument("--transfer-loss-coeff", type=float, default=0.,
                       help="The value of the transfer regularization loss coefficient.")
    group.add_argument("--moe-enable-pre-act-stat", action="store_true",
                       help="Whether to enable the pre-activation statistics.")
    group.add_argument("--moe-use-target-for-loss-free", action="store_true",
                       help="Whether to use the target activations instead of current ones for loss-free load balancing.")
    return parser
