...
from ..utils import get_te_version, init_method_normal, is_te_min_version, scaled_init_method_normal, mup_init_method_normal
...
class TransformerConfig(ModelParallelConfig):
    ...
    use_mup: bool = False
    """Enable mup(Maximal Update Parameterization)"""

    mup_base_hidden_size: int = 256
    """hidden size of small model used in mu-transfer"""

    mup_emb_scale: float = 1.0
    """mup parameterization factor for embedding"""

    mup_depth_scale: float = 1.0
    """mup parameterization factor for model depth"""

    use_span_based_attn: bool = False
    """whether use span-based attn"""

    ####################
    # long-context scaling
    ####################
    rope_type: str = 'longrope'
    """rope type, longrope, llama, yarn"""

    longrope_long_factor: Tuple = None
    """longrope long factor"""
    ...
    standard_init_method: Optional[Callable] = None
    """Method to initialize weights of embedding and lm_head layer. Note that bias is always set to zero. Should be a function that
    takes a single Tensor and initializes it. If None, will be set to
    megatron.core.utils.init_method_normal(init_method_std) which is torch nn init normal with
    mean=0.0 and std=init_method_std."""
    ...
    moe_router_topp: float = 0.5
    """Accumulated scores of experts to route to for each token."""
    ...
    expert_activation_norm: str = "null"
    """The normalization method applied to the expert activations."""

    moe_track_expert_metrics: bool = False
    """If True, instrument routed-expert FFNs to record per-expert
    intermediate-activation and weight Frobenius norms, then publish the
    per-layer max/median and min/median dispersion ratios through the
    standard MoE logging path (TensorBoard / W&B).
    ...
    localized activation blow-up, both of which are invisible to router
    dispatch statistics and to the training loss."""

    moe_expert_metrics_per_layer_logging: bool = False
    """If True, expert health metrics are logged per layer (in addition to a
    cross-layer mean). Only takes effect when ``moe_track_expert_metrics`` is
    True."""

    moe_expert_metrics_per_expert_logging: bool = False
    """If True, expert health metrics emit one scalar per (layer, expert).
    Very verbose — intended for offline diagnosis of suspected dead /
    rogue experts. Only takes effect when ``moe_track_expert_metrics`` is
    True."""

    expert_not_gated: bool = False
    """Whether to make each MoE expert a non-gated FFN."""

    router_type: str = "topk"
    """The type of router to use in MoE layers."""

    dense_mixer_enabled: bool = False
    """If set, use the DenseMixer for training."""

    ####################
    # BlockFFN related
    ####################
    use_blockffn: bool = False
    """Whether to use BlockFFN."""

    router_act_func: str = "relu"
    """The activation function of router."""

    router_norm_type: str = "simple"
    """The type of normalization after router."""

    router_norm_init_var: float = 0.1
    """The init value of normalization after router."""

    router_norm_fixed: bool = False
    """Whether to use a fixed normalization factor after the router."""

    router_norm_scalar: bool = False
    """Whether to use the scalar normalization factor after the router."""

    expert_act_func: str = "norm_silu"
    """The activation function of experts."""

    expert_act_norm_type: str = "normal"
    """The type of normalization inside expert activation functions."""

    expert_act_norm_init_var: float = 1.0
    """The init value of the normalization inside expert activation functions."""

    router_target_act_ratio: float = 0.2
    """The target activation ratio of router."""

    router_clamp_act_ratio: float = 1.0
    """The maximum activation ratio allowed."""

    router_entropy_loss_coeff: float = 0.0
    """The initial value of the router entropy loss coefficient."""

    transfer_loss_coeff: float = 0.0
    """The value of the transfer regularization loss coefficient."""

    moe_use_target_for_loss_free: bool = False
    """Whether to use the target activations instead of current ones for loss-free load balancing."""
        ...
        if self.init_method is None:
            if self.use_mup:
                self.init_method = mup_init_method_normal(
                    self.init_method_std, self.hidden_size / self.mup_base_hidden_size
                )
            else:
                self.init_method = init_method_normal(self.init_method_std)

        if self.standard_init_method is None:
            # only use in use_mup
            self.standard_init_method = init_method_normal(self.init_method_std)

        if self.output_layer_init_method is None:
            if self.use_mup:
                self.output_layer_init_method = mup_init_method_normal(
                    self.init_method_std, self.hidden_size / self.mup_base_hidden_size
                )
            else:
