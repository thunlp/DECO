...
class TransformerLayer(MegatronModule, BaseTransformerLayer):
        ...
        *,
        ...
        with self.bias_dropout_add_exec_handler():
            if self.config.use_mup:
                # mup_bias_dropout_add_fused_train / mup_bias_dropout_add_fused_inference
                hidden_states = self.self_attn_bda(self.training, self.config.bias_dropout_fusion, self.config.use_mup)(
                    attention_output_with_bias, residual, self.hidden_dropout, self.config.mup_depth_scale, self.config.num_layers
                )
            else:
                # bias_dropout_add_fused_train / bias_dropout_add_fused_inference / bias_dropout_add_unfused
    ...
    def _forward_mlp(self, pre_mlp_layernorm_output, residual):
        ...
        with self.bias_dropout_add_exec_handler():
            if self.config.use_mup:
                hidden_states = self.mlp_bda(self.training, self.config.bias_dropout_fusion, self.config.use_mup)(
                    mlp_output_with_bias, residual, self.hidden_dropout, self.config.mup_depth_scale, self.config.num_layers
                )
            else:
