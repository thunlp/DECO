...
class LossFreeTokenL1:

    _update_method: str = "sgd_sign"
    _apply_method: str = "rms"
    ...
    @classmethod
    @torch.no_grad()
    def update_expert_bias(cls, expert_bias: torch.Tensor, tokens_per_expert: torch.Tensor, target_per_expert: torch.Tensor, expert_bias_update_rate: float) -> torch.Tensor:
        ...
        Args:
            ...
            target_per_expert (torch.Tensor): The target activation value of each expert.
        ...
        if cls._update_method == "sgd_sign":
            updated_expert_bias = expert_bias - expert_bias_update_rate * torch.sign(tokens_per_expert - target_per_expert)
        else:
    ...
    def apply_expert_bias(cls, expert_bias: torch.Tensor, router_scores: torch.Tensor) -> torch.Tensor:
        if cls._apply_method == "base":
            ...
            expert_bias = expert_bias.unsqueeze(0) * torch.sqrt(variance)
            scores_for_routing = (router_scores + expert_bias).to(router_scores.dtype)
        else:
