...
from megatron.core.transformer.moe.expert_metrics import (
    record_fc1_pre_activation,
    record_input_activation,
    record_intermediate_activation,
    record_output_activation,
    register_expert_module,
)
...
class GroupedMLP(MegatronModule):
    ...
    def __init__(self, num_local_experts: int, config: TransformerConfig):
        ...
        # MoE health metrics — populated by BaseMoELayer.set_layer_number.
        self.layer_number: Optional[int] = None
        self._track_expert_metrics: bool = bool(
            getattr(self.config, "moe_track_expert_metrics", False)
        )
        if self._track_expert_metrics:
            register_expert_module(self)

    def set_layer_number(self, layer_number: int) -> None:
        """Record this expert layer's transformer-block index for logging."""
    ...
    def forward(self, permuted_local_hidden_states: torch.Tensor, tokens_per_expert: torch.Tensor):
        ...
        if permuted_local_hidden_states.nelement() != 0:
            self._maybe_record_input(permuted_local_hidden_states, tokens_per_expert)
            ...
            self._maybe_record_fc1_pre_act(fc1_output, tokens_per_expert)

            if self.activation_recompute:
                intermediate_parallel = self.activation_checkpoint.checkpoint(
                    self.activation_func, fc1_output
                )
                self._maybe_record_intermediate(intermediate_parallel, tokens_per_expert)
                fc2_output = gg.ops.gmm(intermediate_parallel, w2, tokens_per_expert, trans_b=False)
                self._maybe_record_output(fc2_output, tokens_per_expert)
            ...
            self._maybe_record_fc1_pre_act(h, tokens_per_expert)
            if self.activation_recompute:
                h = self.activation_checkpoint.checkpoint(self.activation_func, h)
                self._maybe_record_intermediate(h, tokens_per_expert)
    ...
    def _maybe_record_input(
        self,
        input_parallel: torch.Tensor,
        tokens_per_expert: torch.Tensor,
    ) -> None:
        """Record routed expert input norms for the per-expert MoE health tracker."""
        if not self._track_expert_metrics:
            return
        if not (self.training and torch.is_grad_enabled()):
            return
        if torch.is_tensor(tokens_per_expert):
            counts = tokens_per_expert.detach().to("cpu").tolist()
        else:
            counts = list(tokens_per_expert)
        record_input_activation(
            layer_number=self.layer_number,
            hidden_states=input_parallel,
            tokens_per_expert=counts,
            num_local_experts=self.num_local_experts,
        )

    def _maybe_record_fc1_pre_act(
        self,
        fc1_pre_act: torch.Tensor,
        tokens_per_expert: torch.Tensor,
    ) -> None:
        """Record fc1 pre-activation and gate/up branch norms."""
        if not self._track_expert_metrics:
            return
        if not (self.training and torch.is_grad_enabled()):
            return
        ...
        record_fc1_pre_activation(
            layer_number=self.layer_number,
            fc1_pre_act=fc1_pre_act,
            tokens_per_expert=counts,
            num_local_experts=self.num_local_experts,
            gated_linear_unit=self.config.gated_linear_unit,
        )

    def _maybe_record_intermediate(
        self,
        intermediate_parallel: torch.Tensor,
        tokens_per_expert: torch.Tensor,
    ) -> None:
        """Record SwiGLU intermediate norms for the per-expert MoE health tracker.

        Skipped under ``no_grad`` so we record at most once per training step
        even when the parent MoE layer is wrapped in ``moe_layer_recompute``.
        """
        if not self._track_expert_metrics:
            return
        if not (self.training and torch.is_grad_enabled()):
        ...
        record_intermediate_activation(
            layer_number=self.layer_number,
            intermediate=intermediate_parallel,
            tokens_per_expert=counts,
            num_local_experts=self.num_local_experts,
        )

    def _maybe_record_output(
        self,
        output_parallel: torch.Tensor,
        tokens_per_expert: torch.Tensor,
    ) -> None:
        """Record routed expert output norms for the per-expert MoE health tracker."""
        if not self._track_expert_metrics:
            return
        if not (self.training and torch.is_grad_enabled()):
            return
...
class MoELayerNorm(torch.nn.Module):
    def __init__(self, dim: int, num_experts: int, eps: float = 1e-6, fixed: bool = True):
        super().__init__()
        self.dim, self.num_experts, self.eps = dim, num_experts, eps
        self.weight = None
        if not fixed:
            self.weight = torch.nn.Parameter(torch.ones(self.num_experts, self.dim, dtype=torch.float32))

    def _norm(self, x):
        return x * torch.rsqrt(x.pow(2).mean(-1, keepdim=True) + self.eps)

    @torch.compile
    def forward(self, x: torch.Tensor, topk_indices: Optional[torch.Tensor] = None) -> torch.Tensor:
        output = self._norm(x.float())
        if self.weight is None:
            return output.type_as(x)
        else:
            return (output * self.weight[topk_indices.view(-1)]).type_as(x)
...
class TEGroupedMLP(MegatronModule):
    ...
    def __init__(self, num_local_experts, config: TransformerConfig, submodules: MLPSubmodules):
        ...
        self.expert_activation_norm = self.config.expert_activation_norm
        self.pre_act_norm = None
        if self.expert_activation_norm != "null":
            self.pre_act_norm = MoELayerNorm(
                dim=ffn_hidden_size if self.expert_activation_norm == "gate_up" else self.config.moe_ffn_hidden_size,
    ...
    def forward(
        ...
        self._maybe_record_fc1_pre_act(intermediate_parallel, tokens_per_expert)

        def bias_act_func(intermediate_parallel, bias_parallel):
            if self.expert_activation_norm == "null":
                pass
            elif self.expert_activation_norm == "gate_up":
                assert self.pre_act_norm is not None
                assert intermediate_parallel.shape[-1] == 2 * self.config.moe_ffn_hidden_size
                intermediate_parallel = self.pre_act_norm(intermediate_parallel)
            elif self.expert_activation_norm == "gate":
                assert self.pre_act_norm is not None
                assert intermediate_parallel.shape[-1] == 2 * self.config.moe_ffn_hidden_size
                intermediate_parallel = torch.chunk(intermediate_parallel, 2, dim=-1)
                intermediate_parallel[0] = self.pre_act_norm(intermediate_parallel[0])
                intermediate_parallel = torch.cat(intermediate_parallel, dim=-1)
            else:
                raise NotImplementedError(f"unsupported expert_activation_norm: {self.expert_activation_norm}")
        ...
        if self.config.fp8:
            ...
            output_tokens_per_expert = actual_tokens_per_expert
        else:
            output_tokens_per_expert = tokens_per_expert
        self._maybe_record_output(output, output_tokens_per_expert)

        return output, output_bias

    def _maybe_record_input(self, input_parallel: torch.Tensor, tokens_per_expert: list) -> None:
        """Record routed expert input norms for the per-expert MoE health tracker."""
        if not self._track_expert_metrics:
            return
        if not (self.training and torch.is_grad_enabled()):
    ...
    def _maybe_record_fc1_pre_act(
        self, fc1_pre_act: torch.Tensor, tokens_per_expert: list
    ) -> None:
        """Record fc1 pre-activation and gate/up branch norms."""
        if not self._track_expert_metrics:
            return
    ...
    def _maybe_record_intermediate(
        self, intermediate_parallel: torch.Tensor, tokens_per_expert: list
    ) -> None:
        """Record SwiGLU intermediate norms for the per-expert MoE health tracker.

        Gated by ``moe_track_expert_metrics``. Skipped under ``no_grad`` so we
        record at most once per training step even when the parent MoE layer
        is wrapped in ``moe_layer_recompute`` (which calls forward once with
        grad disabled and again during the backward recompute).
        """
        if not self._track_expert_metrics:
            return
        if not (self.training and torch.is_grad_enabled()):
    ...
    def _maybe_record_output(self, output_parallel: torch.Tensor, tokens_per_expert: list) -> None:
        """Record routed expert output norms for the per-expert MoE health tracker."""
        if not self._track_expert_metrics:
            return
        if not (self.training and torch.is_grad_enabled()):
