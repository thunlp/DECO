...
def topk_softmax_with_capacity(
    ...
    if score_function == "softmax":
        if use_pre_softmax:
            ...
            if expert_bias is not None:
                scores_for_routing = LossFreeBalance.apply_expert_bias(expert_bias, scores)
            else:
                scores_for_routing = scores
            _, top_indices = compute_topk(scores_for_routing, topk, num_groups, group_topk)
            probs = torch.gather(scores, dim=1, index=top_indices)

            norm_ratio = probs.sum(dim=-1, keepdim=True) + 1e-20
            probs = probs / norm_ratio if topk > 1 else probs
            dense_probs = scores / norm_ratio.detach() if topk > 1 else scores
        else:
            if expert_bias is not None:
                raise NotImplementedError("expert bias is not implemented for post-softmax MoE!!!")
            scores, top_indices = compute_topk(logits, topk, num_groups, group_topk)
            probs = torch.softmax(scores, dim=-1, dtype=torch.float32).type_as(logits)
            dense_probs = None  # not implemented!
    elif score_function == "sigmoid":
        raw_scores = torch.sigmoid(logits.float()).type_as(logits)
        if expert_bias is not None:
            scores_for_routing = LossFreeBalance.apply_expert_bias(expert_bias, raw_scores)
            _, top_indices = compute_topk(scores_for_routing, topk, num_groups, group_topk)
            scores = torch.gather(raw_scores, dim=1, index=top_indices).type_as(logits)
        else:
            scores, top_indices = compute_topk(raw_scores, topk, num_groups, group_topk)
        topk_sum = scores.sum(dim=-1, keepdim=True) + 1e-20
        probs = scores / topk_sum if topk > 1 else scores
        dense_probs = raw_scores / topk_sum.detach() if topk > 1 else raw_scores
    elif score_function == "sigmoid_with_prior":
        raise NotImplementedError("score_function=='sigmoid_with_prior' is not supported in topk_softmax_with_capacity yet.")
    else:
        raise ValueError(f"Invalid score_function: {score_function}")

    if scaling_factor:
        probs = probs * scaling_factor
        dense_probs = dense_probs * scaling_factor
    ...
    if capacity_factor is None:
        ...
        return topk_masked_gates, topk_map, tokens_per_expert, dense_probs
    else:
        # TopK with capacity
        ...
        if pad_to_capacity:
            ...
            final_probs = topk_masked_gates * final_map
        return final_probs, final_map, tokens_per_expert, dense_probs
...
def track_moe_metrics(
    ...
    if writer is not None:
        ...
        for name, loss_list in aux_losses.items():
            ...
            writer.add_scalar(f"moe/{name}", loss_list.sum() / num_moe_layers, iteration)
            ...
            if wandb_writer:
                wandb_writer.log({f"moe/{name}": loss_list.sum() / num_moe_layers}, iteration)
                if per_layer_logging:
                    wandb_writer.log(
                        {
...
def newton_schulz_transformation(X, steps=100, eps=1e-6, coeffs=(3.0, -3.2, 1.2), norm: bool = False):
    a, b, c = coeffs
    ori_X = X
    X = X / (X.norm() + eps)
    ...
    if norm:
        ori_std = torch.std(ori_X).detach()
        new_std = torch.std(X).detach()
        X *= (ori_std / new_std)
    return X


def matrix_inverse_sqrt(sigma, eps=1e-6):
    eigenvalues, eigenvectors = torch.linalg.eigh(sigma)
    eigenvalues = torch.clamp(eigenvalues, min=eps)
    return eigenvectors @ torch.diag(torch.rsqrt(eigenvalues)) @ eigenvectors.mT
