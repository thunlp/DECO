...
from ..transformer.moe.loss_free import LossFreeBalance
# from ..transformer.blockffn.loss_free import LossFreeTokenL1
from ..transformer.transformer_config import TransformerConfig
from ..utils import get_attr_wrapped_model, get_model_config
from ..transformer.moe.moe_utils import save_to_aux_losses_tracker
...
def _update_router_expert_bias(model: List[torch.nn.Module], config: TransformerConfig):
    ...
    for model_chunk in model:
        for module in get_attr_wrapped_model(model_chunk, 'modules')():
            if hasattr(module, 'expert_bias'):
                ...
                if hasattr(module, 'local_targets_per_expert') and module.local_targets_per_expert is not None:
                    targets_per_expert_list.append(module.local_targets_per_expert)
    ...
    #     stacked_targets_per_expert = torch.stack(targets_per_expert_list, dim=0).unsqueeze(-1)
    #     stacked_updated_expert_bias = LossFreeTokenL1.update_expert_bias(
    #         stacked_expert_bias, stacked_tokens_per_expert, stacked_targets_per_expert, config.moe_router_bias_update_rate,
    #     )
    # else:
    stacked_targets_per_expert = None
    if len(targets_per_expert_list) > 0:
        assert len(targets_per_expert_list) == len(expert_bias_list)
        stacked_targets_per_expert = torch.stack(targets_per_expert_list, dim=0).unsqueeze(-1)
    stacked_updated_expert_bias = LossFreeBalance.update_expert_bias(
        stacked_expert_bias, stacked_tokens_per_expert, config.moe_router_bias_update_rate, targets_per_expert=stacked_targets_per_expert,
    )
    ...
    for targets_per_expert in targets_per_expert_list:
        targets_per_expert.zero_()
