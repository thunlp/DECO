...
class LanguageModelEmbedding(MegatronModule):
    ...
    def __init__(
        ...
        self.word_embeddings = tensor_parallel.VocabParallelEmbedding(
            ...
            init_method=self.config.init_method if not self.config.use_mup else self.config.standard_init_method,
    ...
    def forward(self, input_ids: Tensor, position_ids: Tensor, tokentype_ids: int = None) -> Tensor:
        ...
        if self.config.use_mup:
            word_embeddings = word_embeddings * self.config.mup_emb_scale
        if self.add_position_embedding:
