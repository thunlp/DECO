...
class RotaryEmbedding(nn.Module):
    ...
    def __init__(
        ...
        if rope_scaling:
            if rope_type == 'llama':
                ...
                assert longrope_long_factor is not None, 'longrope need longrope_long_factor'
                assert self.seq_len_interpolation_factor is None, 'only one interpolation, but longrope and linear interpolation'
                self.longrope_long_factor = torch.tensor(longrope_long_factor, dtype=torch.float32, device=torch.cuda.current_device())
                assert self.longrope_long_factor.shape == (dim // 2, ), 'misaligned shape for LongRoPE rescale factors: {self.longrope_long_factor.shape}'
            else:
                raise ValueError('unsupport long context type: {self.long_context_type}')
    ...
    def get_freqs_non_repeated(self, max_seq_len: int, offset: int = 0) -> Tensor:
        ...
        if self.rope_scaling and self.rope_type == 'longrope':
            freqs = torch.mul(
                torch.outer(seq, 1.0 / self.longrope_long_factor),
                self.inv_freq
            )
        else:
