    ...
    def __init__(
        ...
        if self.position_embedding_type == 'rope' and not self.config.multi_latent_attention:
            self.rotary_pos_emb = RotaryEmbedding(
                ...
                longrope_long_factor=self.config.longrope_long_factor,
        ...
        if self.post_process or self.mtp_process:
            ...
            self.output_layer = tensor_parallel.ColumnParallelLinear(
                ...
                self.vocab_size,
                config=config,
                init_method=config.init_method if not config.use_mup else config.standard_init_method,
                bias=False,
                skip_bias_add=False,
    ...
    def forward(
        ...
        *,
        ...
        if decoder_input is not None:
            ...
            if self.config.use_span_based_attn:
        ...
        # span-based attention
        if self.config.use_span_based_attn:
            batch_size, max_seqlen = position_ids.shape
            cu_seqlens = torch.nonzero(position_ids.flatten() == 0).flatten()
            cu_seqlens = torch.nn.functional.pad(cu_seqlens, (0, 1), 'constant', batch_size * max_seqlen).type(torch.int32)
            packed_seq_params = PackedSeqParams(
                qkv_format='thd',
                cu_seqlens_q=cu_seqlens,
                cu_seqlens_kv=cu_seqlens,
                max_seqlen_q=position_ids.max() + 1,
                max_seqlen_kv=position_ids.max() + 1,
            )
        ...
        if self.config.use_mup:
            hidden_states = hidden_states / (self.config.hidden_size / self.config.mup_base_hidden_size)
        ...
        if self.config.use_span_based_attn:
            logits = logits.reshape(batch_size, max_seqlen, logits.shape[-1]).transpose(0, 1).contiguous()
